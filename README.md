...
Personal / consulting site for Michael A. Heroux. Plain Jekyll, no build tooling beyond Jekyll
itself — edit Markdown/HTML/CSS directly and push to `master`; GitHub Actions builds and deploys
(see `.github/workflows/pages.yml`).

## Structure

- `index.html` — homepage
- `bio.md`, `consulting.md`, `writing.md` — top-level pages (each sets its own `permalink`)
...
- [`maherou/MJConnections`](https://github.com/maherou/MJConnections) → `/MJConnections/` —
  personal (wedding) site, unrelated to the professional content here.
